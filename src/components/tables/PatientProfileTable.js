...
import { viewPatientAction, editPatientAction, patientModuleSearchTypes } from "../../store/actions/Patient";
...
const mapStateToProps = (state) => {
	...
	if (state.patientModule.searchType === patientModuleSearchTypes.patient && state.patientModule.searchResults !== undefined) {
		profiles = state.patientModule.searchResults;
	}
...
class PatientProfilesTable extends Component {
	constructor(props) {
		...
		const headCells = [
			{ id: "mr", align: "left", disablePadding: false, label: "MR No." },
		...
		if (this.props.profiles.length > 0) {
			dataRows = this.props.profiles.map((patientObj) => {
	...
	componentDidUpdate(prevProps) {
		if (prevProps.profiles === this.props.profiles) return;
		// results clear
		if (this.props.profiles.length === 0) {
			this.setState({
				dataRows: [],
	...
	render() {
		...
		let { page, rowsPerPage, order, orderBy, selected, dense, headCells, dataRows } = this.state;
